...
if( $cur_user['is_guest'] == false ) {
	...
	$tpl_main = str_replace('<acm_status>', '<div id="brdstatus" class="inbox" >'.$lang_common['Acc num'].': '.$cur_user['id'].' '.$lang_common['Prem days'].': '.(int)$cur_user['premdays'].'</div>', $tpl_main);
}
else $tpl_main = str_replace('<acm_status>', '', $tpl_main);
